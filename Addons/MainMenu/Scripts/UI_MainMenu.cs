...
public class UI_MainMenu : MonoBehaviour
{
    ...
    public UI_Settings settingsPanel;                                 //The panel for the options menu.
#endif
...
/*#if _iMMOSETTINGS
        settingsPanel = GameObject.Find("UI_Settings");
...
#if _iMMOSETTINGS
        settingsButton.onClick.AddListener(delegate { settingsPanel.ShowHideSetting(); });                                 //Watches for someone to click the settings button.
#endif
        #endregion _iMMOSETTINGS
    }
    ...
    private void Update()
    {
        ...
        if (mainMenuPanel.activeSelf)
        {
            ...
            changeCharacterButton.interactable = remainingLogoutTime <= 0;
            logoutButton.interactable = remainingLogoutTime <= 0;
            exitButton.interactable = remainingLogoutTime <= 0;
    ...
    public void BtnOnChangeCharacter()
    {
        if (characterCoroutine != null) StopCoroutine(characterCoroutine);
        characterCoroutine = StartCoroutine(OnChangeCharacterToLobby());
    }
    ...
    private IEnumerator OnChangeCharacter()
    {
        ...
        if (!player) yield return new WaitForSeconds(0);                                                //If the player isn't online then return nothing.
...
#if _iMMOZONES
            if(network.networkManagerMMONetworkZones)
                network.networkManagerMMONetworkZones.OnClient_SwitchServerScene(new SwitchServerMsg { characterName = "", scenePath = network.networkManagerMMONetworkZones.listSceneScriptable.CharacterManager.unityScenes.SceneName });
            else
            {
...
#else
            //network.onlineScene = network.networkManagerMMONetworkZones.listSceneScriptable.CharacterManager.unityScenes.SceneName;
            // inputs
    ...
    private IEnumerator OnChangeCharacterToLobby()
    {
        var net = NetworkManager.singleton as NetworkManagerMMO;
        var player = Player.localPlayer;
        if (!net || !player) yield break;

        net.changingCharacters = true;

#if !UNITY_SERVER || UNITY_EDITOR
        // simple modal while we wait
        if (popup != null)
        {
            popup.panel.SetActive(true);
            popup.Show("Switching character...");
        }
        mainMenuPanel.SetActive(false);
#endif

        // ask the server to move this connection back to Lobby
        NetworkClient.Send(new CharacterSwitchRequestMsg());

        // wait until we're in Lobby, or until something goes wrong
        float waited = 0f;
        float timeout = logoutTimer + 5; // reuse your existing budget
        while (waited < timeout)
        {
            if (net.state == NetworkState.Lobby) break;
            if (NetworkClient.connection == null || !NetworkClient.active) break; // disconnected by some other path
            yield return new WaitForSeconds(0.1f);
            waited += 0.1f;
        }

#if !UNITY_SERVER || UNITY_EDITOR
        if (net.state == NetworkState.Lobby)
        {
            // selection screen is now active via OnClientCharactersAvailable
            if (popup != null) popup.panel.SetActive(false);
            mainMenuPanel.SetActive(false);
        }
        else
        {
            // fail-safe: fall back to your old logout flow so nothing breaks
            if (popup != null) popup.panel.SetActive(false);
            logoutCoroutine = StartCoroutine(OnLogout());
        }
#endif

        net.changingCharacters = false;
        characterCoroutine = null;
    }


    public void BtnOnLogout()
    {
        logoutCoroutine = StartCoroutine(OnLogout());
    }
